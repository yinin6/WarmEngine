...
func (bc *BlockChain) GetUpdateStatusTrie(txs []*core.Transaction) common.Hash {
	...
	fmt.Printf("CurrentBlock state hash is  %x\n", bc.CurrentBlock.Header.StateRoot)
	...
	if err != nil {
		bc.chainLog.Println(err)
	}
	err = bc.triedb.Commit(rt, false)
	if err != nil {
		bc.chainLog.Println(err)
	}
	bc.chainLog.Println("modified account number is ", cnt)
	bc.chainLog.Printf("new state hash is  %x\n", rt.Bytes())
	return rt
}
...
func (bc *BlockChain) AddBlock(b *core.Block) {
	if b.Header.Number != bc.CurrentBlock.Header.Number+1 {
		bc.chainLog.Fatalf("the block height is not correct")
		return
	}

	if !bytes.Equal(b.Header.ParentBlockHash, bc.CurrentBlock.Hash) {
		bc.chainLog.Fatalf("err parent block hash")
		return
	}
	...
	bc.chainLog.Println(bc.CurrentBlock.Header.Number+1, "the root = ", rt.Bytes())
	if !bytes.Equal(rt.Bytes(), b.Header.StateRoot) {
		bc.chainLog.Fatalf("the state root is not correct")
	}
	bc.CurrentBlock = b
...
func NewBlockChain(cc *params.ChainConfig, db ethdb.Database) (*BlockChain, error) {
	...
	bc.chainLog = NewLog(cc.ShardID, cc.NodeID)
	bc.chainLog.Printf("The blockchain is generated successfully\n")
...
func NewLog(sid, nid uint64) *log.Logger {
	...
	dirpath := params.LogWrite_path + "/Chain/S" + strconv.Itoa(int(sid))
	...
	fmt.Println("The log is generated successfully")
	return pl
}

func (bc *BlockChain) SetBlockChainDB(db ethdb.Database) {
	bc.db = db
	triedb := trie.NewDatabaseWithConfig(db, &trie.Config{
		Cache:     0,
		Preimages: true,
	...
	bc.triedb = triedb
	// check the existence of the trie database
	_, err := trie.New(trie.TrieID(common.BytesToHash(bc.CurrentBlock.Header.StateRoot)), triedb)
	if err != nil {
		log.Panic()
	}
