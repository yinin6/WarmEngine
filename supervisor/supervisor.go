...
func (d *Supervisor) checkValidIP(address string) bool {
	for _, v := range d.Ip_nodeTable {
		for _, vv := range v {
...
func (d *Supervisor) TcpListen() {
	listenAddr := "0.0.0.0:" + strings.Split(d.IPaddr, ":")[1]
	...
	for {
		...
		if d.checkValidIP(remoteIp) == false {
			fmt.Println("invalid ip address", clientAddr)
			continue
		} else {
			fmt.Println("receive client address:", clientAddr)
		}
