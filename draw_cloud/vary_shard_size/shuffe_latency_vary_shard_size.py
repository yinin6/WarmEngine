...
def get_single_shard_sync_data_of_each_epoch(mod=1, shard_id=0, node_num=10):
    ...
    for i in range(1, move_node_num + 1):
        file_path = f"../../ali_result/result/data=3000000_inj=10000_block=1000_S16N{node_num}/Mod{mod}_Num{move_node_num}_Zone{zone_size}_frq50_band{bandwidth}/S{shard_id}N{i}.csv"
...
def get_single_shard_sync_data_of_each_epoch_of_Proposed(shard_id=0, node_num=5):
    ...
    zone_size = int(move_node_num / 2)
    for i in range(1, move_node_num + 1):
        file_path = f"../../ali_result/result/data=3000000_inj=10000_block=1000_S16N{node_num}/Mod2_Num{move_node_num}_Zone{zone_size}_frq50_band{bandwidth}/S{shard_id}N{i}specific.csv"
