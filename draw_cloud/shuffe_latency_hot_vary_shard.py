...
def get_single_shard_diff_nodes_hot(mod=3, shard_id=0):
    ...
    if mod == 2:
        heatmap = sns.heatmap(heatmap_data, cmap="Blues", cbar=True, vmin=1.5, vmax=4, annot=True, )
    else:
        heatmap = sns.heatmap(heatmap_data, cmap="Blues", cbar=True, vmin=min_value, vmax=max_value, annot=True,
                              fmt=".2f", )
    # 6. 修改颜色条的刻度
    if mod == 2:
        tick_values = np.linspace(min_value, max_value, 8)
...
def get_single_shard_diff_nodes_hot_merge(shard_id=0):
    ...
    mods = [3, 1, 2]
    fig, axes = plt.subplots(nrows=len(mods), figsize=(7, 10 * len(mods)))
    for idx, mod in enumerate(mods):
        ...
        merged_df = merged_df.groupby(['shardID', 'epoch']).agg(
            cost=('cost', 'mean')
        ).reset_index()

        # 2. 创建透视表，将 'epoch' 和 'nodeID' 转换为行和列，'cost' 作为值
        heatmap_data = merged_df.pivot_table(index='shardID', columns='epoch', values='cost')
        ...
        if mod == 2:
            heatmap = sns.heatmap(heatmap_data, ax=ax, cmap="Blues", cbar=True, vmin=1.5, vmax=3.5, annot=True,
                                  annot_kws={'size': 14})
            colorbar = heatmap.collections[0].colorbar
            colorbar.set_ticks([1.5, 2, 2.5, 3, 3.5])
            ax.tick_params(axis='x', labelsize=20)
        else:
            if mod == 1:

                heatmap = sns.heatmap(heatmap_data, ax=ax, cmap="Blues", cbar=True, vmin=10,
                                      vmax=80, annot=True, fmt=".0f", xticklabels=False, annot_kws={'size': 14})
                ...
                heatmap = sns.heatmap(heatmap_data, ax=ax, cmap="Blues", cbar=True, vmin=min_value, vmax=max_value,
                                      annot=True, fmt=".0f", xticklabels=False, annot_kws={'size': 14}, )
                ax.tick_params(axis='x', labelbottom=False)

        ax.set_xlabel("")
        ax.set_ylabel(labels[idx], fontsize=20)
        ax.tick_params(axis='y', labelsize=20)
    plt.subplots_adjust(wspace=0.3, hspace=0.1)
    plt.xlabel("Epoch", fontsize=20)
    plt.tight_layout()
    plt.savefig("hot_vs_diff_shard.pdf")
