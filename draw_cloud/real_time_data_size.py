...
def get_single_shard_sync_data_of_each_epoch_of_Proposed(mod=1, shard_id=0,bandwidth=500000):
    ...
    for i in range(1, move_node_num + 1):
        ...
        file_path = f"../ali_result/result/data=3000000_inj=10000_block=1000_S16N10/Mod{mod}_Num6_Zone3_frq50_band500000/S{shard_id}N{i}specific.csv"
...
def real_time_confirm_tx():
    ...
    file_path = f"../ali_result/result/data=3000000_inj=10000_block=1000_S16N10/Mod2_Num6_Zone3_frq50_band500000/pbft_shardNum=16/Shard00.csv"
...
def draw_time_vs_sync_data_size():
    ...
    plt.xlabel('Running Time (Sec.)', fontsize=font_size)
    plt.ylabel('            Fetched data (Bytes)    Confirm. TXs', fontsize=22)

    # 创建左上角的图例
    legend_top = ax2.legend(lines_top, [f'Shard #{i}' for i in range(0, 10)], loc="upper left", fontsize=16,ncol=2,handletextpad=0.2, columnspacing=0.5)

    # 创建右下角的图例
    legend_bottom = ax2.legend(lines_bottom, [f'Shard #{i}' for i in range(8, 16)], loc="lower right", fontsize=16,ncol=2,handletextpad=0.2, columnspacing=0.5)
