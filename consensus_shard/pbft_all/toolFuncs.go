...
// get neighbor nodes in a shard
func (p *PbftConsensusNode) getLeaderNodes() []string {
	receiverNodes := make([]string, 0)
	for i := 0; i < params.ShardNum; i++ {
		receiverNodes = append(receiverNodes, p.ip_nodeTable[uint64(i)][0])
	}
	return receiverNodes
}
...
func (p *PbftConsensusNode) writeCSVline(metricName []string, metricVal []string) {
	...
	targetPath := fmt.Sprintf("%s/Shard%d%d.csv", dirpath, p.ShardID, p.pbftChainConfig.NodeID)

	p.pl.Plog.Printf("S%dN%d : write to %s\n", p.ShardID, p.NodeID, targetPath)
...
func (p *PbftConsensusNode) writeCSVlineByName(filename string, metricName []string, metricVal []string) {
	// Construct directory path
	dirpath := params.DataWrite_path + "pbft_shardNum=" + strconv.Itoa(int(p.pbftChainConfig.ShardNums))
	err := os.MkdirAll(dirpath, os.ModePerm)
	if err != nil {
	...
	// Construct target file path
	targetPath := fmt.Sprintf("%s/%sS%d%d.csv", dirpath, filename, p.ShardID, p.pbftChainConfig.NodeID)
...
func (p *PbftConsensusNode) RelayMsgSend() {
	...
	for sid := uint64(0); sid < p.pbftChainConfig.ShardNums; sid++ {
		...
		for _, v := range params.SourceNodeID {
			go networks.TcpDial(msg_send, p.ip_nodeTable[sid][uint64(v)])
		}
